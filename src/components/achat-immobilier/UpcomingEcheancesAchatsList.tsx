...
import { useUpcomingEcheancesAchats, type EcheanceAchat } from "@/hooks/useEcheancesAchats";
...
export function UpcomingEcheancesAchatsList() {
  const { data: upcomingEcheances = [], isLoading } = useUpcomingEcheancesAchats();

  const getUrgencyBadge = (ech: EcheanceAchat) => {
    ...
    if (isToday(dueDate)) {
      return (
        <Badge className="bg-destructive/10 text-destructive border-destructive/30">
          <AlertCircle className="h-3 w-3 mr-1" />Aujourd'hui
        </Badge>
      );
    }
    if (daysUntil <= 3) {
      return (
        <Badge className="bg-destructive/10 text-destructive border-destructive/30">
          <Clock className="h-3 w-3 mr-1" />Dans {daysUntil}j
        </Badge>
      );
    }
    if (daysUntil <= 7) {
    ...
    return (
      <Badge variant="secondary">
        <Calendar className="h-3 w-3 mr-1" />Dans {daysUntil}j
      </Badge>
    );
  };
  ...
  const stats = useMemo(() => {
    ...
    const within7Days = upcomingEcheances.filter(e => {
      const d = differenceInDays(new Date(e.due_date), today);
      return d <= 7 && d >= 0;
    }).length;
                ...
                <TableBody>
                  {upcomingEcheances.map((ech) => (
                    <TableRow key={ech.id}>
                      ...
                      <TableCell>{getUrgencyBadge(ech)}</TableCell>
