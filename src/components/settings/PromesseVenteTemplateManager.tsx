...
import {
  usePromesseVenteTemplates,
  useCreatePromesseVenteTemplate,
  useUpdatePromesseVenteTemplate,
  useDeletePromesseVenteTemplate,
  type PromesseVenteTemplate,
} from "@/hooks/usePromesseVenteTemplates";
import {
  DEFAULT_PROMESSE_VENTE_TEMPLATE,
  PROMESSE_VENTE_VARIABLES,
  SAMPLE_PROMESSE_VENTE_DATA,
  replacePromesseVenteVariables,
} from "@/lib/promesseVenteTemplate";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
function PromesseVentePreview({ content }: { content: string }) {
  const previewContent = useMemo(() => {
    if (!content) return "";
    return replacePromesseVenteVariables(content, SAMPLE_PROMESSE_VENTE_DATA);
  }, [content]);
    ...
    if (!previewContent) {
      return (
        <div className="text-center text-muted-foreground py-8">
          ...
          <p>Commencez à rédiger votre promesse pour voir l'aperçu</p>
        </div>
      );
    }
        ...
        <div className="p-6 bg-background">
          <div className="border-b pb-4 mb-6">
            <div className="text-center">
              <h1 className="text-lg font-bold text-primary mb-1">{SAMPLE_PROMESSE_VENTE_DATA.vendeurName}</h1>
              <p className="text-xs text-muted-foreground">{SAMPLE_PROMESSE_VENTE_DATA.vendeurAddress}, {SAMPLE_PROMESSE_VENTE_DATA.vendeurCity}</p>
              <p className="text-xs text-muted-foreground">{SAMPLE_PROMESSE_VENTE_DATA.vendeurPhone} | {SAMPLE_PROMESSE_VENTE_DATA.vendeurEmail}</p>
            </div>
          </div>
          <div className="prose prose-sm max-w-none">{renderPreviewContent()}</div>
          <div className="mt-10 pt-6 border-t grid grid-cols-2 gap-8">
            <div className="text-center">
              <p className="text-sm font-medium mb-1">Le Promettant</p>
              <p className="text-xs text-muted-foreground mb-8">Signature précédée de "Lu et approuvé"</p>
              <div className="border-b border-dashed" />
            </div>
            <div className="text-center">
              <p className="text-sm font-medium mb-1">Le Bénéficiaire</p>
              <p className="text-xs text-muted-foreground mb-8">Signature précédée de "Lu et approuvé"</p>
              <div className="border-b border-dashed" />
            </div>
          </div>
...
export function PromesseVenteTemplateManager() {
  const { toast } = useToast();
  const { data: templates, isLoading } = usePromesseVenteTemplates();
  const createTemplate = useCreatePromesseVenteTemplate();
  const updateTemplate = useUpdatePromesseVenteTemplate();
  const deleteTemplate = useDeletePromesseVenteTemplate();

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [isDeleteDialogOpen, setIsDeleteDialogOpen] = useState(false);
  const [selectedTemplate, setSelectedTemplate] = useState<PromesseVenteTemplate | null>(null);
  const [templateToDelete, setTemplateToDelete] = useState<PromesseVenteTemplate | null>(null);
  ...
  const handleCreateNew = () => {
    ...
    setFormData({
      name: "",
      content: DEFAULT_PROMESSE_VENTE_TEMPLATE,
      is_default: templates?.length === 0,
    });
    setIsDialogOpen(true);
  };

  const handleEdit = (template: PromesseVenteTemplate) => {
  ...
  const handleDuplicate = (template: PromesseVenteTemplate) => {
    setSelectedTemplate(null);
    setFormData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedTemplate) {
        ...
        toast({ title: "Modèle mis à jour", description: "Le modèle de promesse de vente a été mis à jour." });
      } else {
        await createTemplate.mutateAsync(formData);
        toast({ title: "Modèle créé", description: "Le modèle de promesse de vente a été créé." });
      }
  ...
  const handleSetDefault = async (template: PromesseVenteTemplate) => {
    try {
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader><CardTitle>Modèles de promesses de vente</CardTitle></CardHeader>
        <CardContent>
    ...
    <>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Gérez vos modèles de promesses de vente pour les parcelles de lotissements.
              </CardDescription>
            </div>
        ...
        <CardContent>
          {!templates || templates.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="mb-4">Aucun modèle de promesse de vente configuré.</p>
              <Button onClick={handleCreateNew}>
                <Plus className="h-4 w-4 mr-2" />
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{selectedTemplate ? "Modifier le modèle" : "Nouveau modèle de promesse de vente"}</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Ex: Promesse de vente standard..."
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <ScrollArea className="h-32 border rounded-lg p-3">
                <div className="flex flex-wrap gap-2">
                  {PROMESSE_VENTE_VARIABLES.map((v) => (
            ...
            <Tabs defaultValue="editor" className="w-full">
              ...
              <TabsContent value="editor" className="mt-4">
                <Textarea
                  ...
                  placeholder="Rédigez votre promesse de vente ici..."
                />
              </TabsContent>
              <TabsContent value="preview" className="mt-4">
                <PromesseVentePreview content={formData.content} />
              </TabsContent>
            </Tabs>
          </div>
