...
interface TenantPaymentChartProps {
  payments: Array<{
...
export function TenantPaymentChart({ payments }: TenantPaymentChartProps) {
  ...
  const getMonthlyData = () => {
    ...
    // Filter paid payments and sum for each month
    payments
          ...
          <>
            <ChartContainer config={chartConfig} className="h-[200px] w-full">
              <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="tenantRevenueGradient" x1="0" y1="0" x2="0" y2="1">
                ...
                <ChartTooltip
                  content={
                    <ChartTooltipContent
                      formatter={(value) => [`${Number(value).toLocaleString("fr-FR")} F CFA`, "Paiements"]}
                    />
                  }
                />
                <Area
                  ...
                  strokeWidth={2}
                  fill="url(#tenantRevenueGradient)"
                />
              </AreaChart>
            </ChartContainer>
