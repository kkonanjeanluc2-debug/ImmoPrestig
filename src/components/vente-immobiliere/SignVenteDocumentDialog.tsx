...
export function SignVenteDocumentDialog({
  open,
  onOpenChange,
  vente,
  documentType,
}: SignVenteDocumentDialogProps) {
  const { toast } = useToast();
  const { data: agency } = useAgency();
  const { data: signatures, isLoading } = useVenteSignatures(vente.id, documentType);
  const createSignature = useCreateVenteSignature();
  const createBuyerRequest = useCreateBuyerSignatureRequest();

  const [step, setStep] = useState<"overview" | "sign" | "invite">("overview");
  const [buyerEmail, setBuyerEmail] = useState(vente.acquereur?.email || "");
  ...
  const handleBuyerDirectSign = async () => {
    ...
    try {
      await createSignature.mutateAsync({
        ...
        document_type: documentType,
        signer_type: "buyer",
        signer_name: vente.acquereur?.name || "L'Acquéreur",
        signer_email: vente.acquereur?.email || undefined,
        signature_data: signatureData.signatureData,
        signature_text: signatureData.signatureText,
        signature_type: signatureData.type,
      });
  ...
  const handleSendToBuyer = async () => {
    ...
    try {
      const result = await createBuyerRequest.mutateAsync({
        venteId: vente.id,
        documentType,
        buyerName: vente.acquereur?.name || "L'Acquéreur",
        buyerEmail,
      });

      const appOrigin = "https://property-grace.lovable.app";
      const signatureLink = `${appOrigin}/sign-vente?token=${result.token}`;

      const { error: emailError } = await supabase.functions.invoke("send-vente-signature-invite", {
        body: {
          venteId: vente.id,
          buyerName: vente.acquereur?.name || "L'Acquéreur",
          buyerEmail,
          signatureLink,
          propertyTitle: vente.bien?.title || "Bien",
          totalPrice: vente.total_price,
          documentType,
  ...
  const handleCopyLink = async () => {
    if (pendingBuyerRequest?.signature_token) {
      const appOrigin = "https://property-grace.lovable.app";
      const signatureLink = `${appOrigin}/sign-vente?token=${pendingBuyerRequest.signature_token}`;
      await navigator.clipboard.writeText(signatureLink);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Signez électroniquement le {docLabel.toLowerCase()} avec horodatage sécurisé.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6">
            <div className="bg-muted/50 rounded-lg p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <span className="font-medium">{vente.bien?.title}</span>
                <span className="text-muted-foreground">Acquéreur :</span>
                <span className="font-medium">{vente.acquereur?.name}</span>
                <span className="text-muted-foreground">Prix :</span>
                <span className="font-medium">{vente.total_price.toLocaleString("fr-FR")} FCFA</span>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{agency?.name || "Votre agence"}</p>
                  </div>
                </div>
                {vendorSignature ? (
              ...
              <div className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{vente.acquereur?.name}</p>
                  </div>
                </div>
                {buyerSignature ? (
                  <div className="text-right">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      title="Signer directement (en présence de l'acquéreur)"
                    >
                      <PenTool className="h-4 w-4 mr-2" />
                      Signer ici
                    </Button>
            ...
            {vendorSignature && buyerSignature && (
              <Alert className="bg-green-50 border-green-200">
                ...
                <AlertDescription className="text-green-700">
                  Le {docLabel.toLowerCase()} a été signé par les deux parties.
                </AlertDescription>
              </Alert>
            )}
          </div>
        ...
        {step === "sign" && (
          <div className="space-y-6">
            ...
            <SignatureTypeSelector
              signerName={vendorSignature ? (vente.acquereur?.name || "L'Acquéreur") : (agency?.name || "Le Vendeur")}
