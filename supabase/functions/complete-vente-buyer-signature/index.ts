...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Verify token is valid and not yet used
    const { data: existingSignature, error: fetchError } = await supabaseAdmin
      .from("vente_signatures")
      .select("id, vente_id, document_type, signature_data, signature_text")
    ...
    console.error("Error in complete-vente-buyer-signature:", error);
    return new Response(
