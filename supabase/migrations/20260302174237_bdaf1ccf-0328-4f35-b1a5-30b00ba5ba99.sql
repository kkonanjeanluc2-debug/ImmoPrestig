
-- Fix can_access_property to also handle properties created by gestionnaires (where user_id = gestionnaire's id)
CREATE OR REPLACE FUNCTION public.can_access_property(_user_id uuid, _property_user_id uuid, _assigned_to uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
  SELECT 
    -- User is the property owner (agency owner)
    _user_id = _property_user_id
    OR
    -- User is assigned to this property
    _user_id = _assigned_to
    OR
    -- User is admin/owner of the agency that owns this property
    EXISTS (
      SELECT 1 FROM public.agencies a
      JOIN public.agency_members am ON am.agency_id = a.id
      WHERE a.user_id = _property_user_id
      AND am.user_id = _user_id
      AND am.role = 'admin'::app_role
      AND am.status = 'active'
    )
    OR
    -- User is the agency owner and the property was created by a gestionnaire of their agency
    EXISTS (
      SELECT 1 FROM public.agencies a
      JOIN public.agency_members am ON am.agency_id = a.id
      WHERE a.user_id = _user_id
      AND am.user_id = _property_user_id
      AND am.status = 'active'
    )
    OR
    -- User is an admin member and the property was created by another member of the same agency
    EXISTS (
      SELECT 1 FROM public.agency_members am1
      JOIN public.agency_members am2 ON am1.agency_id = am2.agency_id
      WHERE am1.user_id = _user_id
      AND am1.role = 'admin'::app_role
      AND am1.status = 'active'
      AND am2.user_id = _property_user_id
      AND am2.status = 'active'
    )
    OR
    -- Gestionnaire with no specific assignment can see unassigned properties
    (
      _assigned_to IS NULL
      AND EXISTS (
        SELECT 1 FROM public.agencies a
        JOIN public.agency_members am ON am.agency_id = a.id
        WHERE a.user_id = _property_user_id
        AND am.user_id = _user_id
        AND am.status = 'active'
      )
    )
$function$;
